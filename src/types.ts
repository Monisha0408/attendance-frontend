...
export interface User {
  ...
  role: UserRole; department: string | null; is_active: boolean
  is_superadmin: boolean; must_change_password: boolean; created_at: string
}

export interface AttendanceRecord {
  id: string; user_id: string; date: string
  checkin_time: string | null; checkout_time: string | null
  visited_office: boolean | null
  work_mode: WorkMode; status: AttendanceStatus; notes: string | null
  calls_assigned: number | null; calls_closed: number | null
  calls_hold: number | null; calls_next_day: number | null
  daily_update: string | null
  spares_from_office: SpareFromOffice[] | null
  spares_from_outside: SpareFromOutside[] | null
  spares_required: SpareRequired[] | null
  bill_urls: string[] | null
  checkin_latitude?: number | null; checkin_longitude?: number | null
  checkin_location_name?: string | null; checkout_latitude?: number | null
  checkout_longitude?: number | null; checkout_location_name?: string | null
  user?: User
}
...
export interface LeaveRequest {
  ...
  status: LeaveStatus; admin_note: string | null
  reviewed_at: string | null; created_at: string; user?: User
}

export interface DashboardStats {
  total_employees: number; present_today: number; absent_today: number
  on_leave_today: number; office_today: number
}
...
export interface AuditLog {
  ...
  detail: string | null; created_at: string; performed_by_name: string | null
}
